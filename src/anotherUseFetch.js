...
function fetchReducer (state, action) {
    if (action.type === "fetch") {
      ...
      throw new Error('That action isnt supported')
    }
}
...
export default function OtherFetch() {
  ...
  return (
    <div className="box">
      <h3>This is the above hook useFetch re-implemented with a useReducer hook for the fetch actions</h3>
      <h1>{post.title}</h1>
      <p>{post.body}</p>
      {error && <p>{error}</p>}
